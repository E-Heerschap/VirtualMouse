...
\noindent
Figure \ref{fig:vmDevice} represents the \textit{vmDevice} structure which represents a virtual mouse. The \textit{cdev, dev, lock} fields are not pointers as they are individual to the device and hence should only be stored against the single device. However, multiple mouses can use the same protocol or IOCTL mechanics. 
\begin{figure}
	...
	\begin{center}
		...
		...}{ioctl()};
			...
			\umlclass[x=-1]{vmProtocol}{
			+ char* name\\
			+ file\_operations fops\\
			...
		}{};
			
			\umlclass[x=3.5]{file\_operations}{}{(...fops methods)};
			
			\umlclass[y=-3]{vmIOCTL}{};
		\end{tikzpicture}
	\end{center}
\end{figure}
